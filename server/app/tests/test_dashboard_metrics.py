...
def create_invoice(
    ...
    invoice = Invoice(
        ...
        invoice_number=f"INV-{customer_id}-{status}-{issue_date}",
        status=status,
...
def create_payment(db, customer_id: int, payment_date: date, amount: Decimal):
    payment = Payment(customer_id=customer_id, amount=amount, payment_date=payment_date)
    db.add(payment)
    db.flush()
    return payment
...
def test_revenue_dashboard_cash_basis_metrics():
    ...
    create_payment(db, customer.id, date(2024, 5, 15), Decimal("50.00"))
    create_payment(db, customer.id, date(2024, 6, 10), Decimal("70.00"))
    create_payment(db, customer.id, date(2024, 7, 5), Decimal("500.00"))
