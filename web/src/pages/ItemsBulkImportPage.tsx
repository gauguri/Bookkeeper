...
import { ArrowLeft, CheckCircle2, Download, FileSpreadsheet, ShieldCheck, Upload } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { apiFetch } from "../api";

type ImportMode = "CREATE_ONLY" | "UPDATE_EXISTING" | "UPSERT";

type ItemImportFieldSpec = {
  field: string;
  label: string;
  required: boolean;
  description: string;
...
type ItemImportFormatResponse = {
  delimiter: string;
  has_header: boolean;
  required_fields: string[];
  optional_fields: string[];
  fields: ItemImportFieldSpec[];
  sample_csv: string;
  notes: string[];
};
...
type ItemImportResponse = {
  ...
  imported_items: ItemImportRecord[];
};

const importModeOptions: Array<{ value: ImportMode; label: string; description: string }> = [
  { value: "UPSERT", label: "Upsert", description: "Create new items and update existing items in one run." },
  { value: "CREATE_ONLY", label: "Create only", description: "Reject rows where the item already exists." },
  { value: "UPDATE_EXISTING", label: "Update only", description: "Only update existing items and reject new rows." },
];
...
export default function ItemsBulkImportPage() {
  ...
  const [format, setFormat] = useState<ItemImportFormatResponse | null>(null);
  const [preview, setPreview] = useState<ItemImportResponse | null>(null);
  ...
  useEffect(() => {
    apiFetch<ItemImportFormatResponse>("/items/import-format")
      .then((response) => {
        setFormat(response);
  ...
  const runPreview = async () => {
    ...
    setSuccess("");
    try {
      const response = await apiFetch<ItemImportResponse>("/items/import-preview", {
        method: "POST",
        body: JSON.stringify({
          csv_data: csvData,
  ...
  const executeImport = async () => {
    ...
    setSuccess("");
    try {
      const response = await apiFetch<ItemImportResponse>("/items/import", {
        method: "POST",
        body: JSON.stringify({
          csv_data: csvData,
          has_header: hasHeader,
    ...
    <section className="mx-auto w-full max-w-7xl space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="space-y-2">
          <button className="inline-flex items-center gap-2 text-sm text-muted transition hover:text-foreground" onClick={() => navigate("/sales/items")} type="button">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="app-card p-4">
          ...
          <p className="mt-1 text-2xl font-semibold">{totalMappedFields}</p>
        </div>
        <div className="app-card p-4">
        ...
        <div className="app-card space-y-5 p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted">Upload an item template file or paste CSV directly for validation.</p>
            </div>
        ...
        <div className="app-card space-y-5 p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted">Review row outcomes before loading item master data.</p>
            </div>
          ...
          {importedItems.length > 0 ? (
            <div className="rounded-2xl border border-emerald-300/40 bg-emerald-500/10 p-4">
              <div className="flex items-center gap-2 text-emerald-700">
                ...
                <p className="text-sm font-medium">Imported items</p>
              </div>
